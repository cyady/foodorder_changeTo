...
authController.post('/register', async(req,res) => {
    try {
        const isExisting = await User.findOne({email: req.email})
        if(isExisting){
            throw new Error("Alread such an account with this email. Try a new one!")
        }
...
authController.post('/login', async(req, res) => {
    try {
        ...
        // 123456, asdfasdfasdfadgasadfasdf,ascfaerfgasdf
        ...
        const token = jwt.sign({id: user._doc, isAdmin: user.isAdmin}, process.env.JWT_SECRET, {expiresIn: '5h'})
